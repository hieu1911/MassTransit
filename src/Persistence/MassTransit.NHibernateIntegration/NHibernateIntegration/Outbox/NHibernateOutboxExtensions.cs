...
namespace MassTransit.NHibernateIntegration.Outbox
{
    ...
    {
        public static Task AddSend<T>(this ISession session, SendContext<T> context, IObjectDeserializer deserializer,
            ...
            where T : class
        {
            ...
            return session.SaveAsync(outboxMessage, context.CancellationToken);
        }
    }
}
